...
<head>
  ...
  <style>
    ...
    .certificate { max-width: 800px; margin: 0 auto; background: white; padding: 60px; border: 15px solid ${certTierColor.border}; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
    ...
    .title { text-align: center; font-size: 48px; color: ${certTierColor.primary}; margin: 30px 0; font-weight: bold; }
    .subtitle { text-align: center; font-size: 18px; color: #666; margin-bottom: 40px; }
    .content { line-height: 2; font-size: 18px; color: #333; }
    .tier-badge { display: inline-block; background: ${certTierColor.bg}; color: ${certTierColor.primary}; padding: 8px 20px; border-radius: 20px; font-weight: bold; border: 2px solid ${certTierColor.border}; }
    ...
    .footer { text-align: center; margin-top: 50px; padding-top: 30px; border-top: 2px solid ${certTierColor.border}; }
...
<body>
  <div class="certificate">
    ...
    <div class="footer">
      <div class="date">Issued on ${new Date(attempt.completed_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</div>
    </div>
  </div>
</body>
</html>`;

    // Upload updated certificate HTML
    const certFileName = `certificate-${attemptId}.html`;
    await supabaseClient.storage
      .from("certificates")
      .upload(certFileName, new Blob([certificateHTML], { type: "text/html" }), {
        contentType: "text/html",
        upsert: true,
      });

    // Upsert certificate record with current tier
